trigger Trigonstudent on Student__c(before insert){
	List<Student__c> studentlist=Trigger.old;
	set<String> emailset = new set<String>();
    
    for(Student__c s : studentlist){
	    emailset.add(s.Email__c);
    }
	List<Student__c> duplicatestudentlist = [select s.name,s.email__c 
                                               from Student__c   s 
	...
	for(Student__c s:duplicatestudentlist){
      ...
      s.Email__c.addError('Record already exist with this emailid');
           }
  }
}